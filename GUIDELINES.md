## Guidelines to submit the blog in MojoAuth Blog Portal

Here are some guidelines that you must follow while writing the blog for the MojoAuth:

1. First of all create a `blog-file-name.md` file with your blog slug under `content` directory. The name of the file will be the path of your blog.
2. Any relevant images used in the blog should be placed in the `static/assets/images/<blog-file-name>` directory .
...
6. The blog should have the meta tags at the top of your MD files which should have the below properties.
   ...
   - **tags: (optional)** : You can add the relevant the in the Array of string `for eg: ["Tutorial"]`. Please make sure tags should not be more than three.
